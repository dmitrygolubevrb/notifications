...
class EmailProviderMock implements NotificationProviderInterface
{
    ...
    public function send(Notification $notification): ProviderSendResult
    {
        ...
        if (str_contains($message, '__fail_temporary__')) {
            throw new TemporaryProviderException('Email gateway timeout');
        }
        if (str_contains($message, '__fail_permanent__')) {
            throw new PermanentProviderException('Invalid email');
        }

        usleep(random_int(50_000, 200_000));

        return new ProviderSendResult(
            providerRef: sprintf('email-%d-%s', $notification->id, bin2hex(random_bytes(4))),
        );
    }
}
